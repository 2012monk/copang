...
//    @Value("${spring.jwt.secret}")
    ...
    public Optional<String> createAccessToken(WeUser user) {
        Date now = new Date();
    ...
    public Optional<String> createGuestToken() {
        return createAccessToken(WeUser.builder().role(Role.GUEST).build());
    }
    ...
    public Jws<Claims> resolveToken(String token) throws InvalidTokenException {
        try {
            return Jwts.parserBuilder()
                    .deserializeJsonWith(new JacksonDeserializer(Maps.of("user", WeUser.class).build()))
                    .setSigningKey(key)
    ...
    public Optional<WeUser> resolveUserFromToken(String token) {
        Claims claims = null;
        WeUser user = null;
        try{
            claims = getTokenBody(token);
            user = claims.get("user", WeUser.class);
        }catch (Exception  e){
